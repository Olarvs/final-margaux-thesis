...
$column = array('orderId', 'deliveryMethod', 'paymentMethod', 'orderDateTime', 'orderTotal', 'orderStatus');

$query = "SELECT * FROM tbl_order WHERE userId = $userId AND (orderStatus != 'COMPLETED' AND orderStatus != 'CANCELLED')";
...
if (isset($_POST['search']['value'])) {
 ...
 OR orderStatus LIKE "%' . $_POST['search']['value'] . '%" )
 ';
}
...
foreach ($result as $row) {
    ...
    $sub_array[] = '<div class="d-flex flex-row align-items-center gap-2"> <a href="view-order.php?id='.$row['orderId'].'" type="button" class="btn btn-primary" id="getView">VIEW ORDER</a> '.$cancelBtn.' </div>';
    $data[] = $sub_array;
}

...
$query = "SELECT tbl_order.orderId, tbl_order_address.billingFullName, tbl_order_address.billingContactNum, tbl_order.paymentProof, tbl_order.referenceNum, tbl_user.email,tbl_order.pickupDateTime, tbl_order.orderDateTime, tbl_order.orderTotal, tbl_order.orderStatus, tbl_order.gcashNumber, tbl_order_address.nearestLBC
FROM tbl_order
LEFT JOIN tbl_order_address
ON tbl_order.orderId = tbl_order_address.orderId
LEFT JOIN tbl_user
ON tbl_order.userId = tbl_user.user_id
WHERE tbl_order.deliveryMethod = 'DELIVERY' AND tbl_order.courier = 'LBC EXPRESS' AND tbl_order.lbcMode = 'PICK UP' AND tbl_order.paymentMethod = 'GCASH' AND (tbl_order.orderStatus != 'COMPLETED' AND tbl_order.orderStatus != 'CANCELLED')";
...
foreach ($result as $row) {
    ...
    $sub_array[] = $row['nearestLBC'];
    $sub_array[] = date('M d, Y h:i A', strtotime($row['orderDateTime']));
    $sub_array[] = $row['orderTotal'];
    $sub_array[] = $row['orderStatus'];
    $sub_array[] = '<div class="d-flex flex-row align-items-center gap-2"> <button type="button" class="btn btn-primary" id="get_view" data-id="'.$row['orderId'].'">VIEW ORDER</button> <button type="button" class="btn btn-success" id="get_updateLBC" data-id="'.$row['orderId'].'">UPDATE</button> </div>';
    $data[] = $sub_array;
}

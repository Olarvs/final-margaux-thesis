...
$column = array('orderId', 'billingFullName', 'email', 'deliveryMethod', 'paymentMethod', 'orderDateTimeCompleted', 'reason');

$query = "SELECT tbl_order.orderId, tbl_order_address.billingFullName, tbl_user.email, tbl_order.deliveryMethod, tbl_order.paymentMethod, tbl_order.orderDateTimeCompleted, tbl_order.reason
FROM tbl_order
LEFT JOIN tbl_user
ON tbl_order.userId = tbl_user.user_id
LEFT JOIN tbl_order_address
ON tbl_order.orderId = tbl_order_address.orderId
WHERE tbl_order.orderStatus = 'CANCELLED'";

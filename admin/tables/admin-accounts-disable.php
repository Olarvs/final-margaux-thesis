...
$query = "SELECT * FROM tbl_admin WHERE isVerified = 1 AND status = 0 ";
...
foreach ($result as $row) {
    ...
    if ($row['adminId'] != $adminId) {
        $button = '<button type="button" class="btn btn-primary" id="getEnable" data-id="'.$row['adminId'].'">Enable</button>';

<?php
session_start();
if(!isset($_SESSION['admin_forgot_pass_email'])) {
    ?>
<script>
...
<body>
    <div class="row w-100 mx-0">
        <div class="col-md-8 col-lg-6 col-xl-4 mx-auto">
            <div class="auth-form-light text-left py-4 px-4 px-sm-5">
                ...
                <form id="forgotPassVerificationForm" class="pt-3">
                    <div class="form-group">
                        <input type="email" class="form-control form-control-lg" id="email" name="email"
                            placeholder="Email" value="<?= $_SESSION['admin_forgot_pass_email'] ?>" readonly required>
                    </div>
                    <div class="form-group">
                        <input type="tel" maxlength="6" minlength="6" class="form-control form-control-lg" id="otp"
                            name="otp" placeholder="OTP code" required>
                    </div>
                    <div class="mt-3">
                        <button type="submit"
                            class="btn btn-block btn-primary btn-lg text-dark font-weight-bold auth-form-btn"
                            id="forgotPassVerificationBtn">Verify</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    $(document).ready(function() {
        $('#forgotPassVerificationForm').on('submit', function(e) {
            e.preventDefault();

            var form = new FormData(this);
            form.append('forgotPassVerification', true);
            ...
            $.ajax({
                ...
                cache: false,
                beforeSend: function() {
                    $('#forgotPassVerificationBtn').prop('disabled', true);
                    $('#forgotPassVerificationBtn').text('Processing...');
                },
                complete: function() {
                    $('#forgotPassVerificationBtn').prop('disabled', false);
                    $('#forgotPassVerificationBtn').text('Verify');
                },
